...
var Do = (typeof Do === 'undefined')? function(fn){setTimeout(fn, 0);} : Do;
Douban = {};
...
var remote_submit_json = function(form, func, disable, action, withCredentials) {
    ...
    $.post_withck(act, fvalue, function(ret){
        func(ret);
    }, 'json', undefined, withCredentials);
}
...
Douban.init_unfolder = function(o){
    $(o).click(function(){
        ...
        $.ajaxSetup({cache:true}); // fix fullinfo html里含script标签
...
Douban.init_collect_btn = function(o) {
    $(o).click(function(e){
        ...
        url = `${domain ? "https://"+ domain : ""}/j/subject/`+sid+'/interest?'+
            ...
            success: function (r) {
                ...
                $.fn.movieDisplayLimit = function (textarea, ele, limit) {
                    function _setCharactorLeft (objTextarea, objCharactor, numLimit) {
                        var textLen = Math.ceil(objTextarea.val() && objTextarea.val().replace(/[^\x00-\xff]/g, "**").length / 2) || 0
                        if (numLimit < textLen) {
                            objCharactor.html('<strong>' + (numLimit - textLen) + '</strong>')
                        } else {
                            objCharactor.text(numLimit - textLen);
                        }
                    }
                    // 因为半角和英文字符按半个字来计算
                    ...
                    function _setTextareaMaxLength (objTextarea) {
                        ...
                        textarea.attr('maxlength', 350 + Math.ceil(halfWidthCharactorLength / 2));
                ...
                if ($('div#dialog form.movie-sns').length) {
                    $('textarea#comment').unbind().movieDisplayLimit($('textarea#comment'), $('span#left_n'), 350);
                }
            }
        });
...
Douban.init_search_result = function(o){
    if($.suggest) $('#sinput').suggest('/j/subject_suggest',{resultsClass : 'rc_results', onSelect:function(){
        $(o).parent().submit();
    }});
    ...
    setItemList: function (options) {
        ...
        $(CSS_SET_GROUP_LIST).delegate("li:not('.last')", 'click', function (e) {
            ...
            var target = e.target,
                ...
                postPara = (isCb && cb.is(':checked') || !isCb && !cb.is(':checked')) ? 'addtotag' : 'removefromtag';
...
function getslider(nbtn,pbtn,mover,url,pp_id,cat_id,fetch_num){
    var now = 5, max_num = 100, fetch_num = fetch_num || 5, off_set=0,
    slide = function(n){
        if (now+n>max_num){
            now = max_num;
